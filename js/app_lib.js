...
app.get.isDesktop = function() {
	//isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(app.ua),
	//first
	var isDesktop = ('DeviceOrientationEvent' in window || 'orientation' in window) ? false : true;
	if(/(Windows NT|Macintosh|Mac OS X|Linux)/i.test(app.ua)) { isDesktop = true; }
	if(/Mobile/i.test(userAgent)) { isDesktop = false; }
	//second
	var a = app.ua || navigator.vendor || window.opera;

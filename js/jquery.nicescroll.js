(function($) {
	...
	var NiceScrollClass = function(myopt, me) {
		...
		function getZIndex() {
			...
			while (dom.length > 0) {
				if (dom[0].nodeType == 9) return
				false;
				var zi = dom.css('zIndex');
				if (!isNaN(zi) && zi != 0) return
				parseInt(zi);
				dom = dom.parent();
			}
			return false;
		...
		this.doRailClick = function(e, dbl, hr) {
			...
			if (dbl) {
				...
				cur = (hr) ? ((e.pageX - self.railh.offset().left - (self.cursorwidth / 2)) * self.scrollratio.x) : (
		...
		if (!self.hasanimationframe) {
			setAnimationFrame = function(fn) {
				return setTimeout(fn, 15 - Math.floor(
					(+new Date) / 1000) % 16)
			}; // 1000/60)};
...
*/
					...
					if (self.opt.enablemousewheel) {
						if (!self.isiframe) self.bind(
							(cap.isie && self.ispage) ? document : self.win /*self.docscroll*/ , "mousewheel", self.onmousewheel);
				...
				self.onkeypress = function(e) {
					...
					if ($(tg).attr('contenteditable')) return
					true;
			...
			if ((self.page.maxh == premaxh) && (self.page.maxw == premaxw) && (self.view.w == preview.w)) {
				...
				if (!self.ispage) {
					...
					if (self.lastposition) {
						...
						if ((lst.top == pos.top) && (lst.left == pos.left)) return
						self; //nothing to do            
					}
...
*/
		...
		this.getTransitionSpeed = function(dif) {
			...
			var ex = Math.min(sp, Math.round(
				(dif / 20) * self.opt.scrollspeed));
			return (ex > 20) ? ex : 0;
		};
		if (!self.opt.smoothscroll) {
			...
			this.onScrollTransitionEnd = function() {
				...
				self.doScrollPos(px, py, self.opt.snapbackspeed);

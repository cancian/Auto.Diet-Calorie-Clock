...
function appTimer(content) {
	...
	if(app.device.wp8 || app.device.msapp || app.device.android || app.device.blackberry) {
		...
		timerDiff = timerDiff*4;
	}
	//THROTTLE LIMIT
	     if(timerDiff > 2500)	{ timerDiff = 5000; }
	else if(timerDiff > 500)	{ timerDiff = 1000; }
	else if(timerDiff < 100)	{ timerDiff = 200;  }
}
//#////////////////////////#//
...
function updateNutriBars() {
	...
	if(app.read('appNutrientTimeSpan',30) && elapsedDays >= 30) {
		tFii = tFii / 30;
		tSug = tSug / 30;
		tSod = tSod / 30;
	} else if(app.read('appNutrientTimeSpan',7) && elapsedDays >= 7)  {
		tFii = tFii / 7;
		tSug = tSug / 7;
		tSod = tSod / 7;
	} else if(!app.read('appNutrientTimeSpan',1) && elapsedDays >= 1) {

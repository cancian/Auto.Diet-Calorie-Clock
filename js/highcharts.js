...
(function () {
	...
	ta.prototype = {
		...
		init : function (a, b, c, d, e) {
			...
			this.createElement("desc").add().element.appendChild(x.createTextNode(""));
		...
		symbol : function (a,
			...
			j = a.match(i)[1],
			a = Jb[j] || f && f.width && f.height && [f.width, f.height],
			g = this.image(j).attr({
					x : b,
					y : c
				}),
	...
	ia = ma(O, {
			...
			drawPoints : function () {
				...
				q(a.points, function (h) {
					...
					j ? (bb(j), j.attr(i)[b.pointCount < e ? "animate" : "attr"](w(f))) : h.graphic = d[h.shapeType](f).attr(g).attr(i).add(a.group).shadow(c.shadow, null, c.stacking && !c.borderRadius);
					else if (j)
						h.graphic = j.destroy()
				})
			},
	...
	H.pie.prototype.alignDataLabel = sa,
	H.pie.prototype.verifyDataLabelOverflow =
	function (a) {
		var b = this.center,
		c = this.options,
		d = c.center,

/*1393003046,180770389,JIT Construction: v1131751,en_US*/
...
__d("sdk.XD",["sdk.Content","sdk.Event","Log","QueryString","Queue","sdk.RPC","sdk.Runtime","sdk.Scribe","URL","UrlMap","XDConfig","XDM","sdk.createIframe","sdk.feature","guid"],function(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u){var v=new k(),w=new k(),x=new k(),y,z,aa=u(),ba=u(),ca=location.protocol+'//'+location.host,da,ea=false,fa='Facebook Cross Domain Communication Frame',ga={},ha=new k();l.setInQueue(ha);function ia(oa){i.info('Remote XD can talk to facebook.com (%s)',oa);m.setEnvironment(oa==='canvas'?m.ENVIRONMENTS.CANVAS:m.ENVIRONMENTS.PAGETAB);}function ja(oa,pa){if(!pa){i.error('No senderOrigin');throw new Error();}var qa=/^https?/.exec(pa)[0];switch(oa.xd_action){case 'proxy_ready':var ra,sa;if(qa=='https'){ra=x;sa=z;}else{ra=w;sa=y;}if(oa.registered){ia(oa.registered);v=ra.merge(v);}i.info('Proxy ready, starting queue %s containing %s messages',qa+'ProxyQueue',ra.getLength());ra.start(function(ua){da.send(typeof ua==='string'?ua:j.encode(ua),pa,sa.contentWindow,ba+'_'+qa);});break;case 'plugin_ready':i.info('Plugin %s ready, protocol: %s',oa.name,qa);ga[oa.name]={protocol:qa};if(k.exists(oa.name)){var ta=k.get(oa.name);i.debug('Enqueuing %s messages for %s in %s',ta.getLength(),oa.name,qa+'ProxyQueue');(qa=='https'?x:w).merge(ta);}break;}if(oa.data)ka(oa.data,pa);}function ka(oa,pa){if(pa&&pa!=='native'&&!o(pa).isFacebookURL())return;if(typeof oa=='string'){if(/^FB_RPC:/.test(oa)){ha.enqueue(oa.substring(7));return;}if(oa.substring(0,1)=='{'){try{oa=ES5('JSON','parse',false,oa);}catch(qa){i.warn('Failed to decode %s as JSON',oa);return;}}else oa=j.decode(oa);}if(!pa)if(oa.xd_sig==aa)pa=oa.xd_origin;if(oa.xd_action){ja(oa,pa);return;}if(oa.access_token)m.setSecure(/^https/.test(ca));if(oa.cb){var ra=na._callbacks[oa.cb];if(!na._forever[oa.cb])delete na._callbacks[oa.cb];if(ra)ra(oa);}}function la(oa,pa){if(oa=='facebook'){pa.relation='parent.parent';v.enqueue(pa);}else{pa.relation='parent.frames["'+oa+'"]';var qa=ga[oa];if(qa){i.debug('Enqueuing message for plugin %s in %s',oa,qa.protocol+'ProxyQueue');(qa.protocol=='https'?x:w).enqueue(pa);}else{i.debug('Buffering message for plugin %s',oa);k.get(oa).enqueue(pa);}}}l.getOutQueue().start(function(oa){la('facebook','FB_RPC:'+oa);});function ma(oa){if(ea)return;var pa=g.appendHidden(document.createElement('div')),qa=r.create({blacklist:null,root:pa,channel:ba,flashUrl:q.Flash.path,whenReady:function(ra){da=ra;var sa={channel:ba,origin:location.protocol+'//'+location.host,transport:qa,xd_name:oa},ta=q.XdUrl+'#'+j.encode(sa),ua=q.useCdn?'cdn':'www',va=p.resolve(ua,false),wa=p.resolve(ua,true);if(m.getSecure()!==true)y=s({url:va+ta,name:'fb_xdm_frame_http',id:'fb_xdm_frame_http',root:pa,'aria-hidden':true,title:fa,'tab-index':-1});z=s({url:wa+ta,name:'fb_xdm_frame_https',id:'fb_xdm_frame_https',root:pa,'aria-hidden':true,title:fa,'tab-index':-1});},onMessage:ka});ea=true;}var na={rpc:l,_callbacks:{},_forever:{},_channel:ba,_origin:ca,onMessage:ka,recv:ka,init:ma,sendToFacebook:la,inform:function(oa,pa,qa,ra){la('facebook',{method:oa,params:ES5('JSON','stringify',false,pa||{}),behavior:ra||'p',relation:qa});},handler:function(oa,pa,qa,ra){var sa=location.protocol=='https:',ta=q.useCdn?'cdn':'www',ua=p.resolve(ta,sa);return ua+q.XdUrl+'#'+j.encode({cb:this.registerCallback(oa,qa,ra),origin:ca+'/'+ba,domain:location.hostname,relation:pa||'opener'});},registerCallback:function(oa,pa,qa){qa=qa||u();if(pa)na._forever[qa]=true;na._callbacks[qa]=oa;return qa;}};h.subscribe('init:post',function(oa){ma(oa.xdProxyName);var pa=t('xd_timeout');if(pa)setTimeout(function(){var qa=z&&(!!y==w.isStarted()&&!!z==x.isStarted());if(!qa)n.log('jssdk_error',{appId:m.getClientID(),error:'XD_INITIALIZATION',extra:{message:'Failed to initialize in '+pa+'ms'}});},pa);});e.exports=na;});
__d("sdk.Auth",["sdk.Cookie","copyProperties","sdk.createIframe","DOMWrapper","sdk.feature","sdk.getContextType","guid","sdk.Impressions","Log","ObservableMixin","QueryString","sdk.Runtime","sdk.SignedRequest","UrlMap","URL","sdk.XD"],function(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v){var w,x,y=new p();function z(fa,ga){var ha=r.getUserID(),ia='';if(fa)if(fa.userID){ia=fa.userID;}else if(fa.signedRequest){var ja=s.parse(fa.signedRequest);if(ja&&ja.user_id)ia=ja.user_id;}var ka=r.getLoginStatus(),la=(ka==='unknown'&&fa)||(r.getUseCookie()&&r.getCookieUserID()!==ia),ma=ha&&!fa,na=fa&&ha&&ha!=ia,oa=fa!=w,pa=ga!=(ka||'unknown');r.setLoginStatus(ga);r.setAccessToken(fa&&fa.accessToken||null);r.setUserID(ia);w=fa;var qa={authResponse:fa,status:ga};if(ma||na)y.inform('logout',qa);if(la||na)y.inform('login',qa);if(oa)y.inform('authresponse.change',qa);if(pa)y.inform('status.change',qa);return qa;}function aa(){return w;}function ba(fa,ga,ha){return function(ia){var ja;if(ia&&ia.access_token){var ka=s.parse(ia.signed_request);ga={accessToken:ia.access_token,userID:ka.user_id,expiresIn:parseInt(ia.expires_in,10),signedRequest:ia.signed_request};if(ia.granted_scopes)ga.grantedScopes=ia.granted_scopes;if(r.getUseCookie()){var la=ga.expiresIn===0?0:ES5('Date','now',false)+ga.expiresIn*1000,ma=g.getDomain();if(!ma&&ia.base_domain)g.setDomain('.'+ia.base_domain);g.setSignedRequestCookie(ia.signed_request,la);}ja='connected';z(ga,ja);}else if(ha==='logout'||ha==='login_status'){if(ia.error&&ia.error==='not_authorized'){ja='not_authorized';}else ja='unknown';z(null,ja);if(r.getUseCookie())g.clearSignedRequestCookie();}if(ia&&ia.https==1)r.setSecure(true);if(fa)fa({authResponse:ga,status:r.getLoginStatus()});return ga;};}function ca(fa){var ga,ha=ES5('Date','now',false);if(x){clearTimeout(x);x=null;}var ia=ba(fa,w,'login_status'),ja=u(t.resolve('www',true)+'/connect/ping').setSearch(q.encode({client_id:r.getClientID(),response_type:'token,signed_request,code',domain:location.hostname,origin:l(),redirect_uri:v.handler(function(ka){if(k('e2e_ping_tracking',true)){var la={init:ha,close:ES5('Date','now',false),method:'ping'};o.debug('e2e: %s',ES5('JSON','stringify',false,la));n.log(114,{payload:la});}ga.parentNode.removeChild(ga);if(ia(ka))x=setTimeout(function(){ca(function(){});},1200000);},'parent'),sdk:'joey',kid_directed_site:r.getKidDirectedSite()}));ga=i({root:j.getRoot(),name:m(),url:ja.toString(),style:{display:'none'}});}var da;function ea(fa,ga){if(!r.getClientID()){o.warn('FB.getLoginStatus() called before calling FB.init().');return;}if(fa)if(!ga&&da=='loaded'){fa({status:r.getLoginStatus(),authResponse:aa()});return;}else y.subscribe('FB.loginStatus',fa);if(!ga&&da=='loading')return;da='loading';var ha=function(ia){da='loaded';y.inform('FB.loginStatus',ia);y.clearSubscribers('FB.loginStatus');};ca(ha);}h(y,{getLoginStatus:ea,fetchLoginStatus:ca,setAuthResponse:z,getAuthResponse:aa,parseSignedRequest:s.parse,xdResponseWrapper:ba});e.exports=y;});
__d("invariant",[],function(a,b,c,d,e,f){function g(h){if(!h){var i=new Error('Minified exception occured, use the non-minified dev environment for '+'the full error message and additional helpful warnings.');i.framesToPop=1;throw i;}}e.exports=g;});
...
__d("sdk.XFBML.LoginButton",["sdk.Helper","IframePlugin"],function(a,b,c,d,e,f,g,h){var i=h.extend({constructor:function(j,k,l,m){this.parent(j,k,l,m);var n=h.getVal(m,'on_login');if(n)this.subscribe('login.status',function(o){g.invokeHandler(n,null,[o]);});},getParams:function(){return {scope:'string',perms:'string',size:'string',login_text:'text',show_faces:'bool',max_rows:'string',show_login_face:'bool',registration_url:'url_maybe',auto_logout_link:'bool',one_click:'bool',show_banner:'bool'};}});e.exports=i;});
...
} catch (e) {new Image().src="http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1131751","message":"'+e.message+'"}}');}
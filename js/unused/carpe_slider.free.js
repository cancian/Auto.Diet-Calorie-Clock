		$("#entryTitle").val(Number($("#entryTitle").val())+50);
		var sliderPos = document.getElementById('slider').slider.setValue(Number($("#entryTitle").val()));
		
		
			//unlock slider
	$("#sliderNum").on(doubletap, function(evt) {
	//evt.preventDefault();
		alert();
		$("#entryTitle").toggleClass("free");

		if($("#entryTitle").hasClass("free")) {
			window.localStorage.setItem("formSlider","free");
		} else {
			window.localStorage.removeItem("formSlider");
		}
...
(function () {
    ...
    Slider.prototype.snap = function () {
		...
		if(!$("#entryTitle").hasClass("free")) {
			...
			n = document.getElementById('entryTitle').value / 50;
			n = Math.round(n) * 50;
			if($("#entryTitle").val() != n) {
				$("#entryTitle").val(n);
			}
		} else 
			if($("#entryTitle").val() > 25 || $("#entryTitle").val() < 25) {
		$("#entryTitle").val(0);
		this.setPosition();
		//		$("#entryTitle").val(n);
			} else {
...
$(document).ready(function() {
	...
	function carpeLoad() {
		...
		if(window.localStorage.getItem("formSlider") == "free") {
			$("#entryTitle").addClass("free");
		}
	}
	function carpeShow() {
...
$(window).on("orientationchange", function(evt) {	
	...
	function carpeLoad() {
		clearTimeout(carpeTimer);
		$("#sliderWrapper").html('<input id="slider" type="range" min="-750" max="750" step="50" value="0" data-carpe-targets="entryTitle" data-carpe-decimals="0" />');
		CARPE.sliders.init();
		makeRound();

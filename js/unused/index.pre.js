...
////////////////////////
//-> TRANSLATION
//-> 'tapstart', 'tapend', 'tap', 'singletap', 'doubletap', 'taphold',
...
(function startTimer() {
	updateTimer();
	setTimeout(startTimer, 963);
})();
$("#underscroll").html("been dieting for " + dateDiff(window.localStorage.getItem("config_start_time"),(new Date()).getTime()).replace(" ago",""));
/////////////////////
// ADJUST ELEMENTS //
/////////////////////
...
if(hasTouch()) {
	...
	//$("#slider").css("width", "270px");
	//$("#entryListFix").css("maxHeight", "520px");
	//$('#pageSlideCalc').css("top", "5px");
	//$('#pageSlideInfo').css("top", "5px");
	//$('#pageSlideFood').css("top", "5px");
	//$('#pageSlideCalc').css("position", "absolute");
	//$('#pageSlideInfo').css("position", "absolute");
	//$('#pageSlideFood').css("position", "absolute");
	//$("#timerFix").css("marginTop", "32px");
	//$('#timerFix').css("marginLeft", "35%");
	//$("#timerFix").css("border", "5px ridge #ccc");
	//$("#timerFix").css("borderWidth", "5px 5px 0px 5px");
	//$("#timerFix").css("borderRadius", "8px 8px 0px 0px");
	//$('#entryListFix').css("position", "absolute");
	//$('#entryListFix').css("height", "100%");
	//$('#entryListFix').css("top", "51px");
	//$("#entryListFix").css("marginTop", "32px");
	//$('#entryListFix').css("marginLeft", "35%");
	//$("#entryListFix").css("border", "5px ridge #ccc");
	//$("#entryListFix").css("borderWidth", "0px 5px 5px 5px");
	//$('#entryListWrapper').css("min-height", "288px");
	//$('#entryListScroller').css("height", "auto");
	//FULL DISPLAY DESKTOP
	$('#entryListWrapper').css("min-height",                       ($('#afterLoad').height() - (240 + $('#timer').height())) + "px");
	$('#entryListScroller').css("height",                          ($('#afterLoad').height() - (-9 + $('#timer').height())) + "px");
	//iscroll refresh
	/*
...
// PRELOAD SIDE MENUS
//setTimeout(function(){
//	$.get("calc.html", function(data) { $("#pageSlideCalc").html("<div id='sideMenuCalc'>" + data + "</div>"); });
//	$.get("info.html", function(data) { $("#pageSlideInfo").html("<div id='sideMenuInfo'>" + data + "</div>"); });
//},1000);
...
//# STATIC HANDLERS #// tapstart touchmove tapend longTap
//#/////////////////#// taphold doubleTap singleTap swipe
	...
	// ARROW BUTTONS //
	///////////////////
	$("#sliderNum").off(touchstart).on(touchstart, function(evt) {
		evt.preventDefault();
		$("#entryTime").blur();
		$("#entryBody").blur();
	...
	$("#sliderPos").off(touchstart).on(touchstart, function(evt) {
		evt.preventDefault();
	...
	$("#sliderNeg").off(touchstart).on(touchstart, function(evt) {
		evt.preventDefault();
	...
	!function() {
		document.getElementById('entryTitle').update = function() {
			...
			//force reset < 25
//			if(document.getElementById('entryTitle').value != 0 && document.getElementById('entryTitle').value < 25 && document.getElementById('entryTitle').value > -25) {
			...
			function checkTrack() {
				...
				} else if(document.getElementById('entryTitle').value > 0) {
			...
			function checkSubmit() {
				...
				} else if(!$('#entrySubmit').hasClass('submitActive')) {
	...
	$("#configNow").on(tap, function(evt) {
		...
		//if(navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) {
		if(hasTouch()) {
			navigator.notification.confirm("Are you sure?", onConfirmReset, "reset counter (set to now)" , ["ok","cancel"]);
		} else {
	...
	//LONG TAP
	$("#configNow").on(longtap, function() {
		//console.log('wipe all data');
		//CONFIRMATION DIALOG
		//if(navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) {
		if(hasTouch()) {
			navigator.notification.confirm("Are you sure?", onConfirmWipe, "wipe all data" , ["ok","cancel"]);
		} else {
	...
	function onConfirmWipe(button) {
		if(button == 1) {
			...
			$("#entryList").html("<div id='noEntries'><span>no entries</span></div>");
	...
	// AUTOCLOSE n' hide //
	$("#timerTouch,#editableDiv,#entryList,#go,#entryListForm,#entryListWrapper").on(tap, function(evt) {
	//$("#timerTouch,#editableDiv,#entryList,#go,#entryListForm").on(tap + "swipeLeft swipeRight", function(evt) {		
		evt.preventDefault();
		evt.stopPropagation();
		//quick filter performance
		if(Math.round($("#configNow").css("bottom").replace("px","")) == "0") { return; }
		//close with entrywrapper
		if(evt.target.id != "startDateBarToggle" && evt.target.id != "startDate" && evt.target.id != "iconRepeatToggle" && evt.target.id != "startDateBar" && evt.target.id != "iconRepeatToggle") {
			//save on close click
			if($('#startDate').is(':visible') && Math.round($("#configNow").css("bottom").replace("px","")) != "0") {
				$('#startDate').blur();
			}
	...
	//# FOOD SEARCH ICON #//
	//#//////////////////#//
	$("#entryBodySearch").on(touchend,function(evt) {
		evt.preventDefault();
		//evt.stopPropagation();
		//not while editing
	...
	//$("#timerFix, #editableDiv").swipe("option", "threshold", 20);
	//////////////////////
	// PAGESLIDE CLOSER //
	//////////////////////
...
/*
$('#entryListScroller').on("scrollstart", function(evt) {
alert("scrollstart");	
})
$('#entryListScroller').on("scrollend", function(evt) {
alert("scrollend");
})
*/
	...
	$(window).on("orientationchange", function(evt) {
		...
		setTimeout(function() { $('#pageSlideInfo').css("-webkit-transition-duration",".35s");},0);
		//suspend animation
		...
		setTimeout(function() { $('#pageSlideCalc').css("-webkit-transition-duration",".35s");},0);
		//suspend animation
		$('#pageSlideFood').css("-webkit-transition-duration","0");
		$('#pageSlideFood').css("height",($('#entryListScroller').height() - (61)) + "px");
		setTimeout(function() { $('#pageSlideFood').css("-webkit-transition-duration",".35s");},0);
...
$(document).on("pageload", function(evt) {
					...
					if(!$('.editableInput').is(':visible')) {
						if(!$(this).has('input').length) {
							...
							$('#resetBlock').on(touchstart, function(evt) {
								...
								//SHOW DIALOG
								if(hasTouch()) {
									navigator.notification.confirm("Are you sure?", intoTheVoid, "Reset entry value", ["ok","cancel"]);
								} else {
									if(confirm('Reset entry value?')) { intoTheVoid(1); } else { }
								}
								return false;
							});
							/////////////////////
							...
							if(!isNaN($(this).closest("div").attr("id"))) {
								var editableValue = $("#editableInput").val();
								if(editableValue == "food" || editableValue == "exercise") { $("#editableInput").val(''); }
								// FOCUS, THEN SET VALUE
	...
	$("#entryList div").on("swipeleft swiperight",function(event) {
		...
		if(!$('.delete').hasClass('busy')) {
			$('.active').addClass('busy');
			$('.active').removeClass('open');
			$('.active').on('webkitTransitionEnd',function(event) { $('.active').removeClass('busy'); });
			$('.active').removeClass('active');
		}
		//SHOW
		if(!$('#entryList div:animated').length > 0 && !$('.delete').hasClass('busy') && !$('.delete').hasClass('busy') && !$('.editableInput').is(':visible') && !$('#editable').is(':visible') && !$('.editableInput').is(':focus') && !$('#entryBody').is(':focus') && !$('#entryTime').is(':focus')) {
			...
			$('.delete',this).on('webkitTransitionEnd',function(event) { $('.delete').removeClass('busy'); }); 
		}
		//}
		//}
	});
	...
	// GLOBAL HIDE //
	/////////////////
	$("#timerTouch,#entryListForm,#go,#sliderBlock,#entryListWrapper,#entryList").on("tap swipe", function(evt) {
		evt.preventDefault();
		evt.stopPropagation();
...
$(document).on("pageReload", function(evt) {
	...
	$('#pageSlideFood').on('webkitTransitionEnd',function(evt) {
			...
			if(window.localStorage.getItem("foodDbLoaded") != "done") {
				...
				if(!demoRunning) {
					...
					try {
						...
						//import sql
						$.get('diary_food_dump_search.sql',function(sql) {
		...
		// FOOD HTML //
		///////////////
		$("#pageSlideFood").html('<div id="sideMenuFood"><input type="text" id="foodSearch" placeholder="Food search... (per 100g)" /><span id="iconClear">×</span><span id="iconRefresh" class="icon-refresh"></span><div id="foodListWrapper"><div id="foodList"><span id="noMatches">no matches</span></div></div></div>');
		...
		if(window.localStorage.getItem("searchType") == "exercise") {
			$("#foodSearch").attr('placeholder','Exercise search... (30 min)');
		...
		Diary.prototype.searchFood = function(searchSQL,callback) {
			...
			this.db.transaction(
				function(t) {
					t.executeSql("SELECT * FROM diary_food WHERE type == '" + typeTerm + "' AND " + searchSQL + " ORDER BY CASE when term LIKE " + caseStarts + " THEN 0 ELSE 1 END, UPPER(term) LIMIT 30",[],
					function(t,results) {
		...
		function getRecentList() {
			diary.getEntries(function(data) {
				...
				var recentHtml = "";
				if(sortedList == "") { recentHtml += '<div class="searcheable recentItem"><div><em>no entries</em></div></div>'; }
				...
				$("#foodList").html("<div id='recentBlock'><h3 class='recentItem'>Search history</h3>" + recentHtml + "</div>");
		...
		function doSearch(rawInput) {
				...
				diary.searchFood(searchSQL,function(data) {
					...
					for(var s=0, len=data.length; s<len; s++) {
						...
						//if(s <= 100) {
							...
							//html
							var foodLine = "<div class='searcheable' id='" + type + code + "' title='" + kcalBase + "'><div class='foodName'>" + name + "</div><span class='foodKcal'><span class='preSpan'>kcals</span>" + kcal + "<span class='posSpan'>kcals</span></span><span class='foodPro " + typeClass + "'><span class='preSpan'>pro</span>"  + pro  + "<span class='posSpan'>pro</span></span><span class='foodCar " + typeClass + "'><span class='preSpan'>car</span>"  + car  + "<span class='posSpan'>car</span></span><span class='foodFat " + typeClass + "'><span class='preSpan'>fat</span>"  + fat  + "<span class='posSpan'>fat</span></span><span class='foodFib " + typeClass + "'><span class='preSpan'>fib</span>"  + fib  + "<span class='posSpan'>fib</span></span></div>";
							//result list
							foodList += foodLine;
						//}
					} //end loop
			...
			//	tap:function(event) {
			$("#foodList div.searcheable").on(tap,function(evt) {
					event.preventDefault();
					//counting
					ix++;
					var shit = meh;
					...
					if(shit == shot && (duh - deh) < 400 && ix >= 1) {
						...
						//CONTENT TYPE ADJUST
						if(searchType == "food") { 
							$("#modalAmmountType").html('grams');
							$("#modalTotalType").after("<span id='proData'>0.0<span>g</span></span><span id='carData'>0.0<span>g</span></span><span id='fatData'>0.0<span>g</span></span><span id='fibData'>0.0<span>g</span></span><span id='proLabel'>pro</span><span id='carLabel'>car</span><span id='fatLabel'>fat</span><span id='fibLabel'>fib</span>");
						} else {
							$("#modalAmmountType").html('minutes');
						}
						//SHOW MODAL
						...
						function getNutriData() {
							if(searchType == "food") {
								var kcalsPro = Number($("#" + $("#activeOverflow").parent("div").attr("id") + " .foodPro").text().split("pro").join(""));
								var kcalsCar = Number($("#" + $("#activeOverflow").parent("div").attr("id") + " .foodCar").text().split("car").join(""));
								var kcalsFat = Number($("#" + $("#activeOverflow").parent("div").attr("id") + " .foodFat").text().split("fat").join(""));
	...
	$('#iconRefresh').on(touchstart,function(evt) {
		...
		if(!$("#foodSearch").hasClass('busy')) {
			...
			if($("#foodSearch").hasClass("exerciseType")) {
				...
				$("#foodSearch").attr('placeholder','Food search... (per 100g)');
				$("#foodSearch").addClass('busy');
				$("#foodSearch").animate({ backgroundColor: "#ffff99" }, 1).animate({ backgroundColor: "#fff" },600,function() { 
						$("#foodSearch").removeClass('busy');
					}
...
function replaceDivContent(id,content) {
	...
	var cssClass;
	var statusStop;
         if(kcalsInput >  9999) { status = "surplus";  statusStop = " ERROR!"; cssClass = "surplus"; if($("#entryBody").val() != "devilim") { kcalsInput =  9999.99; }}
	else if(kcalsInput < -9999) { status = "deficit";  statusStop = " ERROR!"; cssClass = "deficit"; if($("#entryBody").val() != "devilim") { kcalsInput = -9999.99; }}
	else if(kcalsInput >  600)  { status = "surplus";  statusStop = " STOP!";  cssClass = "surplus";  }
	else if(kcalsInput < -600)  { status = "deficit";  statusStop = " STOP!";  cssClass = "deficit";  }
	else if(kcalsInput >  300)  { status = "surplus";  statusStop = " ";       cssClass = "surplus";  }
	else if(kcalsInput < -300)  { status = "deficit";  statusStop = " ";       cssClass = "deficit";  }
	else                        { status = "balanced"; statusStop = " ";       cssClass = "balanced"; }
	//EQ TIME
	var eqTime;
	...
	var eqTime   = dateDiff(eqDiff*1000,eqDate*1000).replace(" ago","");
	////////////
	// OUTPUT //
	////////////
	var kcalsHtmlOutput = '';
	...
	// SIMPLE DATA //
		kcalsHtmlOutput += "<span id='subStatusEq'>eq. time: <span class='bold'>" + eqTime + "</span></span>";
		kcalsHtmlOutput += "<span id='subKcalsDay'><span class='bold'>" + eqPerDay + " </span> kcals / day</span>";
	// CYCLIC DATA //
	if(content[0] == "cyclic") {
		kcalsHtmlOutput += "<span id='subKcalsRange'><span>" + window.localStorage.getItem("config_kcals_day_1") + "~" + window.localStorage.getItem("config_kcals_day_2") + "</span> kcals range</span>";
		...
		kcalsHtmlOutput += "<span id='subCurrentDay' class='currentDay" + currentDay.toUpperCase() + "'><span id='subDayA'>day <span>a</span></span><span id='subDayB'>day <span>b</span></span><span id='subDayC'>day <span>c</span></span><span id='subDayD'>day <span>d</span></span></span>";
	}

...
function dateDiff(date1,date2) {
	...
	if(minutes == 1) { var lMinutes = " minute "; }
	if(hours   == 1) { var lHours   = " hour ";   }
	if(days    == 1) { var lDays    = " day ";    }

	if(days    > 3)                             { var lHours   = ""; hours   = ""; }
	if(days    > 0)                             { var lMinutes = ""; minutes = ""; }
	if(days    > 0 && hours   > 0)              { var lDays    = lDays  + "and ";  }
	if(hours   > 0 && minutes > 0)              { var lHours   = lHours + "and ";  }
	if(days == 0 && hours == 0 && minutes == 0) { minutes = 0; var lMinutes = " minutes "; }

	return days + lDays + hours + lHours + minutes + lMinutes + 'ago';
}
//////////
// TRIM //
//////////
...
function spinner(size) {
	...
	if(!$("#tempHolder").html()) { 
		...
		$("#tempHolder").html('<div id="modalOverlay"><span id="spinnerMsg">preparing database</span></div><div id="spinnerWrapper"><div id="spinner"><div class="bar1"></div><div class="bar2"></div><div class="bar3"></div><div class="bar4"></div><div class="bar5"></div><div class="bar6"></div><div class="bar7"></div><div class="bar8"></div><div class="bar9"></div><div class="bar10"></div><div class="bar11"></div><div class="bar12"></div></div></div>');
...
//if(!navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) {
	var myScroll;
	function loaded() {
		myScroll = new iScroll('entryListFix', {
	...
	var myScroll2;
	function loaded2() {
		myScroll2 = new iScroll('pageSlideInfo', {
			//scrollbarClass: 'myScrollbar',
			useTransform: false,
			zoom: false,
			hideScrollbar: true,
	...
	//onready
	window.addEventListener('load', setTimeout(function () { loaded(); loaded2(); }, 750), false);
}
*/
///////////////////
// PAGE LOAD MOD //
...
function updateEntries(partial) {
	diary.getEntries(function(data) {
		...
		for(var i=0, len=data.length; i<len; i++) {
			...
			var dataHandler = "<div data-id='" + data[i].id + "' id='" + data[i].id + "' class='" + rowClass + "' name='" + dataPublished + "'><p class='entriesTitle'>" + dataTitle + "</p><p class='entriesKcals'>kcals</p><p class='entriesBody'>" + dataBody + "</p><p id='" + dataPublished + "' class='entriesPublished'> " + dateDiff(dataPublished,(new Date()).getTime()) + "</p><span class='delete'>Delete</span></div>";
			// ROW++
			s += dataHandler;
...
var touchstart  = hasTouch() ? ' touchstart  ' : ' tapstart    ';
var touchend    = hasTouch() ? ' touchend    ' : ' tapend      ';
var touchmove   = hasTouch() ? ' touchmove   ' : ' touchmove   ';
var tap         = hasTouch() ? ' tap         ' : ' click       ';
var singletap   = hasTouch() ? ' singletap   ' : ' click       ';
var doubletap   = hasTouch() ? ' doubletap   ' : ' dblclick    ';
var longtap     = hasTouch() ? ' taphold     ' : ' longtap     ';
var swipe       = hasTouch() ? ' swipe       ' : ' swipe       ';
var swipeend    = hasTouch() ? ' swipeend    ' : ' swipeend    ';
var swipeup     = hasTouch() ? ' swipeup     ' : ' swipeup     ';
var swipedown   = hasTouch() ? ' swipedown   ' : ' swipedown   ';
var swipeleft   = hasTouch() ? ' swipeleft   ' : ' swipeleft   ';
var swiperight  = hasTouch() ? ' swiperight  ' : ' swiperight  ';
var scrollstart = hasTouch() ? ' scrollstart ' : ' scrollstart ';
var scrollend   = hasTouch() ? ' scrollend   ' : ' scrollend   ';
var mouseout    = hasTouch() ? ' mouseout    ' : ' mouseout    ';
var orientationchange  = hasTouch() ? ' orientationchange     ' : ' orientationchange ';
/////////////////////////////

...
$.ajax({
	method: 'GET',
	url: app.https + 'world.openfoodfacts.org/cgi/search.pl?json=1&search_terms=' + searchSQL + '&search_simple=1&action=process&time=' + app.now(),
	cache: false,
	
	
	
	//data: { search_terms: searchSQL, json: 1, page_size: 50 }
})
  .done(function( data ) {
    ///alert( "Data Saved: " + msg );
  

//$.ajax({type: 'POST', dataType:  'text', url: app.https + 'world.openfoodfacts.org/cgi/search.pl',  data: { search_terms: encodeURI(searchSQL), json: 1, jqm: 1, action: 'process' }})
//.done: function(data) {
	app.toast(searchSQL);
...
/*
	...
	var itemDatu  = dataJSON.products;
	if(!itemDatu)  { return 'no results'; } 
...
var listing = data.products;
var OLIST = [];
for (var y = 0; y < listing.length; y++) {


var itemData = listing[y];
var nutriData = itemData.nutriments;

var listName = itemData['product_name'] ? itemData['product_name'] : itemData['generic_name'];

//nutriData = itemData.nutriments
//generic_name
...
code: itemData['code'],
name: listName,
term: searchalize(listName),
type: "0100",
kcal: nutriData['energy_100g'],
pro: nutriData['proteins_100g'],
car: nutriData['carbohydrates_100g'],
fat: nutriData['fat_100g'],
fib: "0.00",
fii: nutriData['fiber_100g'],
sug: nutriData['sugars_100g'],
sod: nutriData['sodium_100g'],
}
...
//}
//f($('#searchContents').is(':visible')) {
	onlineList = app.handlers.buildRows(OLIST);
